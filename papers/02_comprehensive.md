...
**Layer 1 — Methylated but expression-protected TFs.** All 27 Hox genes have zero DMPs. DeepTFactor CNN-predicted TFs (Kim et al., 2021) are significantly enriched for DMPs (Fisher's OR = 1.607, *P* = 4.32 × 10^−13^) and DMRs (OR = 1.645, *P* = 3.33 × 10^−6^). However, 0/6 DE TFs carry any methylation changes — methylation targets TFs based on network position but does not alter their expression. Top methylated TFs: ZFP2 (30 DMPs), Mafb (25), ATF2 (18).
