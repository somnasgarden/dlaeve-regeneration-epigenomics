...
#SBATCH --output=slurm_logs/dedup_%A_%a.out  # Standard output log
#SBATCH --error=slurm_logs/dedup_%A_%a.err   # Standard error log
...
echo "Starting Deduplication of BAMs to remove excessive PCR amplification..."
...
MAIN_OUTPUT_DIR="08_deduplication" # Main output folder
...
deduplicate_bismark --outfile "${CURRENT_ID}_paired_bismark_bt2_pe" --output_dir $MAIN_OUTPUT_DIR $INPUT_BAM
